...
mkdir ~/tmp_script
mkdir ~/tmp_log
set tmp_scr =  ~/tmp_script/${var}_atm_assm_NUDGE2_tmp.csh
set tmp_log =  ~/tmp_log/${var}_atm_assm_NUDGE2_tmp.log
...
foreach OBS ( \${OBS_SET} ) #OBS loop1
  foreach FACTOR ( \$FACTOR_SET ) #FACTOR loop1
    foreach mbr ( \$mbr_set ) #mbr loop1
      ...
      set SAVE_ROOT = /mnt/lustre/proj/earth.system.predictability/ATM_NUDGE2_ASSM_EXP/archive_transfer/atm/\${outfreq}/${var}/\${CASENAME} 
      mkdir -p \$SAVE_ROOT

      set input_head = \${ARC_ROOT}/atm/hist/\${CASENAME_M}.cam.h2.????-??-??-?????.nc
