...
foreach OBS ( \${OBS_SET} ) #OBS loop1
  foreach FACTOR ( \$FACTOR_SET ) #FACTOR loop1
    foreach mbr ( \$mbr_set ) #mbr loop1
      ...
      set SAVE_ROOT = /mnt/lustre/proj/earth.system.predictability/ATM_NUDGE2_ASSM_EXP/archive_transfer/ocn/\${outfreq}/${var}/\${CASENAME} 
      mkdir -p \$SAVE_ROOT
      ...
      foreach inputname ( \${input_set} )
        ...
        cdo -O -w -z zip_5 select,name=${var} \$inputname ${homedir}/test_NUDGE2_atm_assm_extract_ocn_${var}.nc
        cdo -O -w -z zip_5 -sellevel,500/15000 ${homedir}/test_NUDGE2_atm_assm_extract_ocn_${var}.nc  \$outputname 
      end
    end
  end
end
rm -f ${homedir}/test_NUDGE2_atm_assm_extract_ocn_${var}.nc
