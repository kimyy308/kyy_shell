...
foreach iyloop ( ${IY_SET} )
  ...
  foreach fy ( ${FY_SET} )
    foreach mon ( 01 02 03 04 05 06 07 08 09 10 11 12 )
      foreach ci ( 1 2 3 )
        if ( ${ci} == 1 ) then
          set inputname = ${ARC_ROOT[${ci}]}/${Chl_set[${ci}]}_${RESOLN}.hcst.${CASENAME}.pop.h.${fy}-${mon}.nc  
        else
          set inputname = ( ${inputname} ${ARC_ROOT[${ci}]}/${Chl_set[${ci}]}_${RESOLN}.hcst.${CASENAME}.pop.h.${fy}-${mon}.nc )
        endif
      end
      set outputname = ${SAVE_ROOT}/sumChl_${RESOLN}.hcst.${CASENAME}.pop.h.${fy}-${mon}.nc
