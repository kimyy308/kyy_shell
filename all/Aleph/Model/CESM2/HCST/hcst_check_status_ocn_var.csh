...
foreach comp ( ${comps} )
  ...
  foreach var ( ${vars} )
    ...
    set CASENAME_SET = ( `ls ${VAR_ROOT} | grep ${RESOLN}.hcst.` ) 
    foreach CASENAME ( ${CASENAME_SET} )
      set fin_period = `du -sh ${VAR_ROOT}/${CASENAME}/*i???? | grep -v 4.0K | rev | cut -d 'i' -f 1 | rev | head --lines=1`
      echo ${var}", "${CASENAME}", i"${fin_period} >> ${stat_ext}
    end
    set fin_period = `du -sh ${VAR_ROOT}/ens_all/*i???? | grep -v 4.0K | rev | cut -d 'i' -f 1 | rev | head --lines=1`
    echo ${var}", "ens_all", i"${fin_period} >> ${stat_ens}
  end
end
