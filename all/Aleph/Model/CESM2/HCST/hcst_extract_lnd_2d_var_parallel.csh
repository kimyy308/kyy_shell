    ...
    foreach OBS ( \${OBS_SET} ) #OBS loop1
      foreach FACTOR ( \$FACTOR_SET ) #FACTOR loop1
        foreach mbr ( \$mbr_set ) #mbr loop1
          ...
          set ARC_ROOT = /mnt/lustre/proj/earth.system.predictability/HCST_EXP/archive/\${CASENAME_M}/\${CASENAME}/lnd/hist/
          set ARC_TS_ROOT = /mnt/lustre/proj/earth.system.predictability/HCST_EXP_timeseries/archive/\${CASENAME_M}/\${CASENAME}/lnd/proc/tseries/month_1
          set SAVE_ROOT = /mnt/lustre/proj/earth.system.predictability/HCST_EXP/archive_transfer/lnd/${var}/\${CASENAME_M}/\${CASENAME} 
          mkdir -p \$SAVE_ROOT
          ...
          foreach fy ( \${FY_SET} )
            ...
            if ( \${#period_SET} == 0 ) then
              foreach mloop ( \${M_SET} )
                set inputname = \${ARC_ROOT}/\${RESOLN}.hcst.\${OBS}-\${FACTOR}p\${mbr}_i\${INITY}.clm2.h0.\${fy}-\${mloop}.nc
                set outputname = \${SAVE_ROOT}/${var}_\${CASENAME}.clm2.h0.\${fy}-\${mloop}.nc
...
#                  @ tind = \`expr \( \${iyloop} - \${Y_1} \) \* 12\`
                  ...
                  foreach mloop ( \${M_SET} )
                    @ tind2 = \$tind + \$mloop
                    #echo \$tind2
                    set inputname = \${ARC_TS_ROOT}/\${CASENAME}.clm2.h0.${var}.\${period_f}.nc
                    set outputname = \${SAVE_ROOT}/${var}_\${CASENAME}.clm2.h0.\${fy}-\${mloop}.nc
                    #ls \$input_head
